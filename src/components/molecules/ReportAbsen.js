...
export default function ReportAbsen({
  ...
  return (
    <div className={['flex space-x-4 pt-2'].join(' ')}>
      <img
        onClick={(event) => handlerClickImage(event)}
        className="h-16 w-16 object-cover cursor-pointer shadow-md shadow-zinc-200/50 rounded-lg"
        src={
          image === '' || image === null
      ...
      />
      <div
        className={['flex flex-col space-y-1 w-5/6', type === 'out' && ''].join(
          ' ',
        )}>
